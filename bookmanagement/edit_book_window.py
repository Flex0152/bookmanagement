...
class EditBookWindow:
    ...
    def modify_book(self):
        new_author = self.cmb_author.get()
        new_title = self.in_title.get()
        new_genre = self.cmb_genre.get()

        try:
            self.db.update_book(self.book_obj.id, new_title, new_author, new_genre)
